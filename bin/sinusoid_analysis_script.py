...
subjdirs = [
    op.join(expdir, s) for s in os.listdir(expdir) if s.startswith("sub")
]
...
for subjdir in subjdirs:
    ...
    ranges["diameter_pc"] = (
        ranges.diameter / baselines.diameter - 1
    ).values * 100
    ranges["diameter_3dpc"] = (
        ranges.diameter_3d / baselines.diameter_3d - 1
    ).values * 100
    ...
    averages = (
        ranges.reset_index().groupby(by=["stim", "onset"], as_index=True).mean()
    )
    ...
    ax.axhspan(-60, -62, color="k", alpha=0.3)
    ...
    metrics = averages.groupby(level=0).agg(
        ...
        L=(
            "diameter_3d",
            lambda s: plr.latency_to_constriction(s, sample_rate, onset, pc),
        ),
        PC=("diameter_3d", lambda s: plr.peak_constriction(s, onset)),
        MCA=(
            "diameter_3d",
            lambda s: plr.maximum_constriction_amplitude(s, onset),
        ),
        MCT=(
            "diameter_3d",
            lambda s: plr.time_to_peak_constriction(s, sample_rate, onset),
        ),
        CT=(
            "diameter_3d",
            lambda s: plr.constriction_time(s, sample_rate, onset, pc),
        ),
        MCV=(
            "diameter_3d",
            lambda s: plr.maximum_constriction_velocity(s, sample_rate),
        ),
    )
