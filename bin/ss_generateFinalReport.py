...
pupildata = pandas.DataFrame(
    np.where(np.asarray(pupildata) == 0, np.nan, np.asarray(pupildata))
)
...
calPerWl = np.asarray(
    pandas.read_csv(
        os.path.dirname(os.path.realpath(__file__)) + "/oo_calibration.csv",
        header=-1,
    )
)
sensorAreaCm2 = float(
    pandas.read_csv(
        os.path.dirname(os.path.realpath(__file__)) + "/oo_sensorArea.csv",
        header=-1,
    ).iloc[0, 0]
)

# Read in CMFs
cies026 = np.transpose(
    np.asarray(
        pandas.read_csv(
            os.path.dirname(os.path.realpath(__file__)) + "/cies026.csv",
            usecols=[1, 2, 3, 4, 5],
        )
    )
)
# cie1931xyz2deg = 683*np.transpose(np.asarray(pandas.read_csv('code/lin2012xyz10e_1_7sf.csv', usecols=[1, 2, 3], header=-1)))
cie1931xyz2deg = 683 * np.transpose(
    np.asarray(
        pandas.read_csv(
            os.path.dirname(os.path.realpath(__file__)) + "/cie1931xyz2.csv",
            header=-1,
        )
    )
)
...
darkcal = pandas.read_csv(
    "darkcalibr_poly31_V2.txt", sep="\t", header=5, index_col=False
)
...
for afile in (
    df1["Original file"].str.split(".", expand=True)[0] + ".csv"
).iteritems():
    spechead = pandas.read_csv(
        sys.argv[1] + "/spectra/" + afile[1], nrows=4, header=-1
    )
    ...
    darkSpd = np.where(
        darkcal.SELECT_RMSE > SELECT_RMSE_THRESHOLD, np.nan, darkSpd
    )
    ...
    # Resample
    uWPerCm2PerNm = spectres.spectres(
        np.arange(380, 781),
        np.asarray(specdf["Wavelengths (nm)"]),
        uWPerCm2PerNm,
    )
    ...
    alphaopicIrradiance = np.dot(
        np.nan_to_num(cies026), np.nan_to_num(WPerM2PerNm) * 1000
    )
