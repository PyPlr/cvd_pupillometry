...
if __name__ == "__main__":
    ...
    # video
    video = "led_test_on_off.dsf"
    apy.load_video_file(device, video)
    ...
    # run video
    video_start_trigger  = new_trigger(video_on, duration_on) # make on trigger
    t = threading.Thread(target=detect_light_onset, args=(video_start_trigger,20))
    t.start()
    sleep(1.)
    send_trigger(video_start_trigger)
    apy.play_video_file(device) # turn on the light
    for l in range(18):
        sleep(1.)
        on_trigger = new_trigger(label_on, duration_on)
        send_trigger(on_trigger)
        sleep(1.)
        off_trigger = new_trigger(label_off, duration_off)
        send_trigger(off_trigger)
