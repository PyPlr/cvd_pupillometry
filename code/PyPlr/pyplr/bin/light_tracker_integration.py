...
if __name__ == "__main__":
    ...
    # a bright light for plr (direct it at a wall...)
    primaries = [4095, 4095, 4095, 4095, 4095, 4095, 4095, 4095, 4095, 4095]
    ...
    for light in range(3):    
        on_trigger  = new_trigger(label_on, duration) # make on trigger
        send_trigger(on_trigger) # send on trigger
        apy.set_spectrum_a(device, [primaries]) # turn on the light
        sleep(.5)
        off_trigger = new_trigger(label_off, duration) # make off trigger
        send_trigger(off_trigger) # send off trigger
        apy.turn_off(device) # turn off the light

        sleep(10.) # wait ten seconds

    sleep(2.)  # wait for recovery
