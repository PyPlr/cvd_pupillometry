...
def based_noise_blinks_detection(samples, sampling_freq):
    """
    ...
    d = {'start_timestamp': samples.iloc[blinks['blink_onset']].index,
         'end_timestamp': samples.iloc[blinks['blink_offset']].index}
    df = pd.DataFrame(data=d)
    df["duration"] = df["end_timestamp"] - df["start_timestamp"]
    print("{} blinks detected by Hershman algorithm, average duration {:.3f} s".format(len(df), df.duration.mean()))
