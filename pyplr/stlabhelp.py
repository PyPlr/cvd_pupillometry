...
def _get_metadata(df, creator="jtm", **metadata):
    default = {"creation_time": str(datetime.now()), "creator": creator}
...
def _video_file_end(end_time):
    df = pd.DataFrame()
    df = df.append(
        _video_file_row(time=end_time)
    ).append(  # two extra dummy rows ensure the light
        _video_file_row(time=end_time + 100)
    )  # turns off when video file finishes
    return df
...
def pulse_protocol(
    ...
    df = pd.DataFrame()
    df = (
        df.append(_video_file_row(0, pulse_spec))
