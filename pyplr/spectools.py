...
def spectra_to_dominant_wavelength(
    spectra, binwidth, ref_white=[0.3333, 0.3333]
):
...
def explore_spectra(spectra, binwidth):
    ...
    for i, led in enumerate(ax):
        ...
        ax[i, 2].plot(
            inpt, pwl.loc[i, "wavelength"], color=colors[i], lw=1, label="Peak"
        )
        ...
        low = (
            dwl.loc[i, "wavelength"].min()
            - dwl.loc[i, "wavelength"].min() * 0.1
        )
        high = (
            dwl.loc[i, "wavelength"].max()
            + dwl.loc[i, "wavelength"].max() * 0.1
        )
